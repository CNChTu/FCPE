...
from fcpes.pcmer import PCmer
from fcpes.pcmer_siren import PCmer as PCmerS
from fcpes.pcmer_full import PCmer as PCmerF
from fcpes.pcmer_siren_full import PCmer as PCmerFS
from fcpes.nvSTFT import STFT
...
class TransformerF0BCE(nn.Module):
    def __init__(
        ...
        self.hop_size = hop_size if (hop_size is not None) else 512
        self.windows_size = windows_size if (windows_size is not None) else 1024
        ...
        self.pre = nn.Linear(windows_size, input_channel)
    ...
    def forward(self, audio, infer=True, gt_f0=None, return_hz_f0=False, cdecoder=None):
        """
        input:
            B x n_frames
        return:
        ...
        """
        audio = torch.nn.functional.pad(audio, (int(self.hop_size // 2), int((self.hop_size + 1) // 2)), mode = 'reflect')
        audio = torch.nn.functional.unfold(audio[:,None,None,:],(1,self.windows_size),stride=self.hop_size).transpose(1,2)
        audio = self.pre(audio)
        ...
        if self.use_input_conv:
            x = self.stack(audio.transpose(1, 2)).transpose(1, 2)
        else:
            x = audio
        x = self.decoder(x)
        x = self.norm(x)
