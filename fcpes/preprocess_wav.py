...
def preprocess(path, f0_extractor, wav2mel, uv_interp=False, read_sr=16000, device='cuda', extensions=['wav']):
    ...
    def process(file):
        ...
        if len(f0[~uv]) > 0:
            ...
            np.save(path_npaudiofile, npaudiofile)
        else:
...
if __name__ == '__main__':
    ...
    preprocess(args.data.train_path, f0_extractor, wav2mel, uv_interp=args.data.us_uv, read_sr=args.mel.sampling_rate, device=device, extensions=['wav'])

    # preprocess validation set
    preprocess(args.data.valid_path, f0_extractor, wav2mel, uv_interp=args.data.us_uv, read_sr=args.mel.sampling_rate, device=device, extensions=['wav'])
