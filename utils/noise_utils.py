...
from src.audio_augmentation import params2sos
from scipy.signal import sosfilt

Qmin, Qmax = 2, 5


def add_noise(wav, noise_ratio=0.7, brown_noise_ratio=1.0):
...
def add_noise_slice(
    wav, sr, duration, add_factor=0.50, noise_ratio=0.7, brown_noise_ratio=1.0
):
    ...
    for n in range(n_frames):
        ...
        if wav[offset : offset + slice_length_noise].shape[0] != 0:
            wav[offset : offset + slice_length_noise] = add_noise(
                wav[offset : offset + slice_length_noise],
    ...
    for n in range(n_frames):
        ...
        for i in range(block_num):
            ...
            if vertical_offset:
                v_offset = int(
                    random.uniform(
                        line_num * i, line_num * (i + 1) - now_vertical_factor
                    )
                )
                n_v_down = v_offset
            ...
            if (
                mel[n_v_down:n_v_up, offset : offset + slice_length_noise].shape[-1]
                != 0
            ):
                mel[
                    n_v_down:n_v_up, offset : offset + slice_length_noise
                ] = add_mel_mask(
                    mel[n_v_down:n_v_up, offset : offset + slice_length_noise],
                    iszeropad,
                    esp,
                )
    if islog:
...
def worldSynthesize(wav, target_sr=44100, hop_length=512, fft_size=2048, f0_in=None):
    f0, t = pyworld.dio(
        wav.astype(np.double), fs=target_sr, frame_period=1000 * hop_length / target_sr
    )
    ...
    synthesized = pyworld.synthesize(
        f0, sp, ap, target_sr, frame_period=1000 * hop_length / target_sr
    )
...
def add_noise_slice_snb(wav, sr, duration, add_factor=0.50, snb=0.7, beta=1.0):
    ...
    for n in range(n_frames):
        ...
        offset = random.randint(left, right - slice_length_noise)
        if wav[offset : offset + slice_length_noise].shape[0] != 0:
            wav[offset : offset + slice_length_noise] = add_noise_snb(
                wav[offset : offset + slice_length_noise], snb, beta
            )
    return wav
