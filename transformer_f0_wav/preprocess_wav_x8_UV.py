...
def parse_args(args=None, namespace=None):
    ...
    # run  
def process(file,path_srcdir,path_f0dir,path_npaudiodir,path_skipdir,path_worlddir,read_sr,uv_interp,f0_extractor,uv_extractor,hop_size):
    binfile = file + '.npy'
    ...
    # extract UV
    uv_h = uv_extractor.extract(audio, uv_interp=False, sr=temp_sr)
...
def loop_process(filelist, path_srcdir,path_f0dir,path_npaudiodir,path_skipdir,path_worlddir,read_sr,uv_interp,f0_extractor,uv_extractor,hop_size=160):
    for file in tqdm(filelist, total=len(filelist)):
        process(file,path_srcdir,path_f0dir,path_npaudiodir,path_skipdir,path_worlddir,read_sr,uv_interp,f0_extractor,uv_extractor,hop_size)

def preprocess(path, f0_extractor,uv_extractor, wav2mel, uv_interp=False, read_sr=16000, device='cuda', extensions=['wav'], num_workers = 4,hop_size=160):
    ...
    with ProcessPoolExecutor(max_workers=num_workers) as executor:
       ...
       for i in range(num_workers):
           ...
           tasks.append(executor.submit(loop_process, file_chunk,path_srcdir,path_f0dir,path_npaudiodir,path_skipdir,path_worlddir,read_sr,uv_interp,f0_extractor,uv_extractor,hop_size))
...
if __name__ == '__main__':
    ...
    preprocess(args.data.valid_path, f0_extractor,uv_extractor, None, uv_interp=args.data.us_uv, read_sr=args.mel.sampling_rate, device=device, extensions=['wav'], num_workers = num_workers,hop_size=args.mel.hop_size)
